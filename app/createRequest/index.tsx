...
import { emptyService, fmtTime, now } from "@/utils";
...
const overallFromServices = (
  s: Record<ServiceKey, ServiceState>,
): RequestStatus => {
...
function reducer(state: RequestData[], action: Action): RequestData[] {
  switch (action.type) {
    ...
    case "SET_OVERALL":
      return state.map((r) =>
        r.id === action.id ? { ...r, overall: action.overall } : r,
      );
    case "CANCEL":
      return state.map((r) =>
        r.id === action.id ? { ...r, overall: "canceled", canceled: true } : r,
      );
...
async function simulateService(
  service: ServiceKey,
  // eslint-disable-next-line no-unused-vars
  onStep: (data: Partial<ServiceState>) => void,
) {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    onStep({
      status: i === 0 ? "processing" : pct >= 100 ? "completed" : "processing",
      progress: pct,
    });
  }
  // örnek sonuç url
  onStep({
    resultUrl: `https://example.com/${service}.jpg`,
    status: "completed",
    progress: 100,
  });
}
...
export default function CreateRequestScreen() {
  ...
  const isBusy = useMemo(
    () =>
      state.some(
        (r) => r.overall === "uploading" || r.overall === "processing",
      ),
    [state],
  );
  ...
  const startProcess = async () => {
    ...
    await simulateUpload((p) => {
      (["serviceA", "serviceB", "serviceC"] as ServiceKey[]).forEach((svc) =>
        dispatch({
          ...
          id,
          service: svc,
          data: { status: "uploading", progress: p },
        }),
      );
    });
    ...
    await Promise.all(
      (["serviceA", "serviceB", "serviceC"] as ServiceKey[]).map(
        async (svc) => {
          try {
            await simulateService(svc, (data) =>
              dispatch({ type: "UPDATE_SERVICE", id, service: svc, data }),
            );
          } catch (e: any) {
            dispatch({
              type: "UPDATE_SERVICE",
  ...
  const retryFailedServices = async (id: string) => {
    ...
    if (!target) return;
    const failed = (Object.keys(target.services) as ServiceKey[]).filter(
      (k) => target.services[k].status === "error",
    );
              ...
              <View style={styles.reqRow}>
                ...
                <View style={{ flex: 1 }}>
                    ...
                    {(Object.keys(req.services) as ServiceKey[]).map((key) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {key.toUpperCase().replace("SERVICE", "S")} •{" "}
                            {c.text}{" "}
                  ...
                  <View style={styles.actionsRow}>
                    {req.overall === "processing" ||
                    req.overall === "uploading" ? (
                      <ThemedButton
                        variant="outline"
                        size="sm"
                    ...
                    {req.overall === "error" ? (
                      <ThemedButton
                        variant="outline"
                        size="sm"
                        onPress={() => retryFailedServices(req.id)}
                      >
                        <Ionicons
                          name="refresh"
                          size={16}
                        ...
                        <ThemedText
                          variant="caption"
                          style={{ marginLeft: 6, color: Colors.warning }}
                        >
                    ...
                    {req.overall === "completed" ? (
                      <ThemedButton
                        size="sm"
                        onPress={() =>
                          Alert.alert("Sonuçlar", "Servis sonuçlarını göster…")
                        }
                      >
                        <Ionicons name="eye" size={16} color={Colors.white} />
                        <ThemedText
                          variant="caption"
                          style={{ marginLeft: 6, color: Colors.white }}
                        >
                          Sonuçları Gör
                        </ThemedText>
                      </ThemedButton>
